...
function suppress(id) {
  if ( confirm("Voulez vous vraiment supprimer ce mat�riel?")) {
...
if ( $operation == 'update' ) {
	...
	$result=mysql_query($query); 
	$row=@mysql_fetch_array($result);
	// si lot de mat�riel, ne peut pas �tre inclus dans un lot (�viter les hi�rarchies)
	//if ( $row[0] == 1 ) {
        //$query="update materiel set MA_PARENT=null where MA_ID=".$MA_ID; 
        //$result=mysql_query($query);
	//}
	
	// si plus lot, enlever les pi�ces de mat�riel attach�es
	//else
	 if ( $row['TM_LOT'] == 0 ){
	...
	if (!empty($MA_PARENT)) {
	// Si le mat�riel n'est plus op�rationnel, le lot non plus'
