...
// month loop
for ( $m = 1; $m <= 12 ; $m++ ) {
    $query ="select count(*) as nb0, sum(E_NB1) as nb1, sum(E_NB2) as nb2 , sum(E_NB3) as nb3
...
$queryN="select TB_LIBELLE from type_bilan where TE_CODE='".$type."' and TB_NUM=1";
$resultN=mysql_query($queryN);
$rowR=@mysql_fetch_array($resultN);
$LIB1=$rowR["TB_LIBELLE"];

$queryN="select TB_LIBELLE from type_bilan where TE_CODE='".$type."' and TB_NUM=2";
$resultN=mysql_query($queryN);
$rowR=@mysql_fetch_array($resultN);
$LIB2=$rowR["TB_LIBELLE"];

$queryN="select TB_LIBELLE from type_bilan where TE_CODE='".$type."' and TB_NUM=3";
$resultN=mysql_query($queryN);
$rowR=@mysql_fetch_array($resultN);
$LIB3=$rowR["TB_LIBELLE"];
...
$textBoxObj = $c->addTitle($TITLE." ".$year, "arialbd.ttf", 15,0xffffff);
...
# symbols
$dataSetObj = $layer->addDataSet($data0, $colors[3], $TITLE);
$dataSetObj->setDataSymbol(PolygonShape(3), 11);
$dataSetObj = $layer->addDataSet($data1, $colors[1], $LIB1);
$dataSetObj->setDataSymbol(DiamondSymbol, 11);
$dataSetObj = $layer->addDataSet($data2, $colors[2], $LIB2);
$dataSetObj->setDataSymbol(CircleSymbol, 9);
$dataSetObj = $layer->addDataSet($data3, $colors[4], $LIB3);
$dataSetObj->setDataSymbol(CircleSymbol, 9);
